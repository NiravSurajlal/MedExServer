...
public class MongoDBUtil {
    ...
    public static MongoDatabase createConnection()
	{
    	try
        {        
       // dbURL = CommonUtil.GetXMLData(Paths.get(path.toString(), "src", "test", "java", "DBSettings.xml").toString(),"MongoDB_DbURL");
			 dbURL = CommonUtil.GetXMLData(Paths.get(path.toString(), "src", "test", "java", "DBSettings.xml").toString(),"MongoDB_DbURL");
		        DBname=CommonUtil.GetXMLData(Paths.get(path.toString(), "src", "test", "java", "DBSettings.xml").toString(),"MongoDB_DbName");
	...
	public static boolean VerifyDataInMongoDB(String queryParam,String value)
	{
        ...
        while (iterator.hasNext()) {
            ...
            if(doc.containsValue(value))
            {
            	...
            	Reporter.addStepLog("Value verified in MongoDB is: " + value);
            	Reporter.addStepLog("MongoDB collection: " + doc.toString());
            }
        }
	...
	{		
		String collectioName=collection;
		 MongoDatabase mydatabase = createConnection();
        /**** Get collection / table from DB ****/
        Document document = mydatabase.getCollection(collectioName).find(dbObject).sort(Sorts.descending("timestamp")).first();
      //  Document document = mydatabase.getCollection(collectioName).find(dbObject)
	...
	{		
        ...
        /**** Get collection / table from DB ****/
        Document document = mydatabase.getCollection(dbDeatails[0]).find(dbobject).sort(Sorts.descending("timestamp")).first();
    ...
    else
    {
	 ...
	 //	 org.bson.Document document=common.MongoDBUtil.GetMongoDBData(dbDeatails[0], dbobject);
	 	 if(dbDeatails.length==3)
	 	 {
	 		String res="";
	 	 String[] outputDataFields= dbDeatails[2].split(",");
	 	 for (int i = 0; i < outputDataFields.length; i++) {
	 	// System.out.println("MongoDB collection: " + document.get(outputDataFields[i]).toString());
	 		 try {
...
//		 query1.put("orderid", "123");
		 
		 
		 FindIterable<Document> cursor = mydatabase.getCollection(dbDeatails[0]/*collectioName*/).find(/*query1*/dbobject);
		 String value = "";
		 for(Document doc : cursor) {
			 
         	Reporter.addStepLog("MongoDB collection: " + doc.toString());
			/* System.out.println(doc.toString());
			 System.out.println(doc.get(dbDeatails[2]));*/
			 value = (String) doc.get(dbDeatails[2]);
			 if(value!=null)
				 break;
		 }
		 String compareValue=DbHelper.queryCopiedText(queryValue[1]).toUpperCase();
