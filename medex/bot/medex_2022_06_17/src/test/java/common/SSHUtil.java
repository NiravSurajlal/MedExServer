...
//					Paths.get(path.toString(), "src", "test", "java", "SSHSettings.xml").toString(), filePath);
			...
			Reporter.addStepLog("************************************************"+host+"  "+user+"  "+password );
		} catch (Exception ex) {
		...
		System.out.println("SPLIT@@@@@@@@@@@@@@ VALUE1*******"+splitValue[4]);
		    ...
		    for(String cmd : commands1)
		    {
		    	...
		    	if(cmd.contains("_delay_"))
		    	{
		    		...
		    		Reporter.addStepLog("Executed command : "+cmds[0]);
			...
			Reporter.addStepLog("Executed command : "+cmd.trim());
		    	 shellStream.flush();
...
// if(fileoutput.contains(verifyText.toUpperCase()))
		...
		System.out.println("Inside VERIFIED DISPLAYED________****false****");
		 ...
		 System.out.println("Inside VERIFIED NOT DISPLAYED________****true****");
		    ...
		    Reporter.addStepLog("Value read from file : '" + fileoutput+ "' , Copied value to be verified : '"+verifyText+"' , Verification type : '"+splitValue[3]+"'");
		    channel.disconnect();
		    session.disconnect();
			}
			else
...
private static boolean containsIgnoreCase(List<String> list, String soughtFor) {
	...
	public static String GetDataFromFile(String filepath)
	{
        ...
        try {
			...
			Reporter.addStepLog("Connected to the host: '" + filePath + "'");
 ...
 return line;
	...
	public static String runCommand(String command) {
        ...
        try {
            ...
            Reporter.addStepLog("Executed the command on Remote machine");
            while(readByte != 0xffffffff)
            {
               outputBuffer.append((char)readByte);
               readByte = commandOutput.read();
            }
            Reporter.addStepLog("Command output: '" + outputBuffer.toString() + "'");
